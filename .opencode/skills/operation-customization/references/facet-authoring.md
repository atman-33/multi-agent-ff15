...
- Supported dynamic placeholders in instructions currently include `{{ output("<step>", "latest", "<artifact>") }}` for prior workflow artifacts and `{{ setting("language", "name") }}` for configured report-language wording.

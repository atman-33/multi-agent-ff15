...
20. Supported instruction placeholders must resolve during prompt composition. `{{ output("step", "selector", "file") }}` must resolve to an existing mission-scoped file, and `{{ setting("language", "name") }}` must use a supported setting key and mode pair. Treat unresolved, malformed, or unsupported placeholders as blocking failures.
...
- Re-check any authored instruction placeholder such as `output(...)` or `setting("language", "name")` against the runtime contract it expects.
...
3. Use supported dynamic placeholders only when the runtime contract guarantees them. `{{ output("<step-name>", "latest", "<artifact-name>") }}` is for prior workflow artifacts, `{{ setting("language", "name") }}` is for configured language wording, and `{{ root("app_root") }}` / `{{ root("execution_root") }}` are for app-owned versus execution-target paths.

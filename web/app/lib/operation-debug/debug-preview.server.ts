...
import type { AgentId, StepHistoryEntry, WorkerAgentId, WorkflowNext } from "@/lib/types/mission";
...
export function buildOperationDebugBundle(input: {
  ...
  for (let index = 0; index < operation.steps.length; index += 1) {
    ...
    const reportMessage = `${reportMessageBase}\n\n[step:${step.name}]`;
    ...
    const ruleEvaluation = [
      ...
      `allowed_next: ${step.rules.map((rule) => rule.next).join(", ") || "(none)"}`,
      "",
      "--- report message ---",
      reportMessage,
      "",
